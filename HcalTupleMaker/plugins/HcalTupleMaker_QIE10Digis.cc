...
{ 
  ...
  produces<std::vector<int>   >                  ( "QIE10DigiCapIdOk"   );
...
void HcalTupleMaker_QIE10Digis::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  std::auto_ptr<std::vector<int> >                    CapIdOk( new std::vector<int>   ());
  ...
  //
  bool debug = false;
  if(debug)printf("qie10Digis->size(): %d\n",qie10Digis->size());
...
///////////////////////////////////////////// Ahmad33
    ...
    if(debug)printf("number of time slices: %d\n",nTS);
    ...
    { 
      ...
      if(debug)printf("soi: %d ok: %d adc: %d fc: %g le_tdc: %d te_tdc: %d capid: %d\n",qie10df[its].soi(),qie10df[its].ok(),qie10df[its].adc(),adc2fC_QIE10[qie10df[its].adc()],qie10df[its].le_tdc(),qie10df[its].te_tdc(),qie10df[its].capid());
      ...
      if(its!=nTS-1){
        if(qie10df[its].capid()!=3){
          if(qie10df[its].capid()+1!=qie10df[its+1].capid())capidok=0;
        }
        else{
          if(qie10df[its+1].capid()!=0)capidok=0;
        }
      }
      else{
        if(qie10df[its].capid()!=3){
          if(qie10df[its].capid()+1!=qie10df[0].capid())capidok=0;
        }
        else{
          if(qie10df[0].capid()!=0)capidok=0;
        }
      }

      //printf("cap id: %d    ",qie10df[its].capid());
  ...
  iEvent.put( CapIdOk       , "QIE10DigiCapIdOk"   );
